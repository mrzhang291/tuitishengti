...
7.C[详解]依题可知， $f'(x) = ae^{x} - \frac{1}{x} \geq 0$ 在(1,2)上恒成立，显然 $a > 0$ ，所以 $x\mathrm{e}^x \geq \frac{1}{a}$ ，
...
10. AD[详解]A 选项：设 $x_{1} > x_{2}$ ，函数 $2^{x}$ 单调递增，所有 $2^{x_{1}} > 2^{x_{2}}$ ， $x_{1} - x_{2} > 0$ ，
...
11. AD[详解]A选项， $f'(x)=6x^{2}-6ax=6x(x-a)$ ，由于a>1，故 $x\in(-\infty,0)\cup(a,+\infty)$ 时 $f'(x)>0$ ，故 $f(x)$ 在 $(-∞,0),(a,+∞)$ 上单调递增， $x∈(0,a)$ 时， $f'(x)<0$ ， $f(x)$ 单调递减，则 $f(x)$ 在x=0处取到极大值，在x=a处取到极小值，由 $f(0)=1>0$ ， $f(a)=1-a^{3}<0$ ，则 $f(0)f(a)<0$ ，根据零点存在定理 $f(x)$ 在 $(0,a)$ 上有一个零点，
...
14. $[\frac{3}{2e}, 1)$ [详解]由题意可知存在唯一的整数 $x_0$ ，使得 $e^{x_0}(2x_0 - 1) < ax_0 - a$ ，设
